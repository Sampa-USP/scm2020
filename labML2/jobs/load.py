...
# Training model
model = createRF(X, y) # create and train
# model = pickle.load(open('rf_model.pkl', 'rb')) # already trained?
...
#Plot 1 -Most important features of the random forest model
...
# Only the (20) most relevant features will be plotted
xg = included[indices][0:20]
yg = importances[indices][0:20]

plt.rcParams.update({'font.size': 14})
plt.barh(xg, yg, align='center', color=c0, alpha=1)
plt.xlabel('Relative importance (%)', fontsize=18)

plt.subplots_adjust(left=0.46, bottom=0.16, right=0.94)

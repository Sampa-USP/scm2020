...
def searchNN(X, y):
    ...
    nrange = []
    for x in range (MAX_NEURONS):
        nrange.append((int(random.random()*(MAX_NEURONS))))
        if (len(nrange) > SEARCH):
            break

    print ('Neurons per layer available to be tested: {}'.format(nrange))

    param_grid = {
    'neurons_hidden_1': nrange,
    'neurons_hidden_2': nrange,
    'neurons_hidden_3': nrange}
